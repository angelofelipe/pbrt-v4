...
namespace pbrt {
    ...
    public:
        ...
        UnderwaterIntegrator(int maxDepth, Camera camera, Sampler sampler, Primitive aggregate, const std::array<Float, 4> samplingVolume,
                             std::vector<Light> lights, Light sunLight = nullptr,
                             const std::string &lightSampleStrategy = "bvh",
                             bool regularize = false)
            : VolPathIntegrator(maxDepth, camera, sampler, aggregate, lights, lightSampleStrategy, regularize),
              sun(sunLight), volumeUniformDistance(samplingVolume.at(0)), volumeUniformSPP(samplingVolume.at(1)),
              volumeVariableSPP(samplingVolume.at(2)), causticsTime(samplingVolume.at(3))
        {
            LOG_VERBOSE("\n\nUnderwaterIntegrator created\n\n");
            SetOnePixelToRender();  // To use in development in this way: if (onePixelToRender) { Printf("[ waterDepth: %f ]\n\n", waterDepth); }
            settingWaterDepthAndMedium();
            LOG_VERBOSE("[ waterDepth: %f ]\n\n", waterDepth);

            if (volumeUniformDistance == 0.f || volumeUniformSPP == 0) { Options->disableSingleScatteringVolumeUniform = true; }
            if (volumeVariableSPP == 0) { Options->disableSingleScatteringVolumeVariable = true; }
            if (Options->disableSingleScatteringVolumeUniform && Options->disableSingleScatteringVolumeVariable) {
    ...
    protected:
        ...
        SampledSpectrum SampleLdUnderwater(const Interaction &intr, const BSDF *bsdf,
                                 SampledWavelengths &lambda, Sampler sampler,
                                 SampledSpectrum beta, SampledSpectrum inv_w_u,
        ...
        static  Float GetTParamIntersectPlaneWater(const Point3f &origin, const Vector3f &direction, const Float &planeWater);

        static Float CausticPattern(const Point2f &p, const Float &time);

        static Float GetCaustics(const Point3f &p, const Vector3f &sunDir, const Float &tToSurface, const Float &time = 1.f);
